...
import djangoinvoicing.validators
...
class Customer(models.Model):
    ...
    company_vat = models.CharField(verbose_name=_("TVA"), validators=[djangoinvoicing.validators.VatValidator], max_length=255, blank=True, null=True)
    ...
    contact_phone = models.CharField(verbose_name=_("Téléphone"), validators=[djangoinvoicing.validators.PhoneValidator], max_length=255)
...
# Quote Models
def generate_quote_number(country_prefix=1):
    """
    Generate quote_number for the Quote. The number should follow each other.
    """
    code_prefix = timezone.now().strftime("%Y%m")
    last_quotes = Quote.objects.filter(quote_number__startswith=code_prefix).values_list('quote_number', flat=True)
    if len(last_quotes) == 0:
        return "{code_prefix}{code}".format(code_prefix=code_prefix, code=format(1, '04d'))
    else:
        next_code = max(map(int, [i[-4:] for i in last_quotes])) + 1
...
class Quote(models.Model):
    ...
    # Quote informations
    quote_number = models.CharField(verbose_name=_("Numéro"), max_length=255, unique=True, default=generate_quote_number)
    quote_date = models.DateField(verbose_name=_("Date de facturation"), default=timezone.now)
    ...
    quote_pdf = models.FileField(verbose_name=_("Document PDF"), blank=True, null=True)
    ...
    def get_address(self):
        ...
        if self.address_bp:
            address += " / BP %s" % self.address_bp
        address += "\n%s - %s" % (self.address_zipcode, self.address_city)
        return address
    get_address.short_description = _("Adresse")

    def get_prices(self):
        excl_tax_vat = {}  # calculate excl_tax foreach vat
        for quoteitem_instance in self.quoteitem_set.all():
            # Vat
            vat_key = str(quoteitem_instance.product_vat)
            if vat_key not in excl_tax_vat:
                excl_tax_vat[vat_key] = Decimal('0.00')
            # Price
            excl_tax_vat[vat_key] += quoteitem_instance.product_price * quoteitem_instance.product_quantity
...
class QuoteItem(models.Model):
    # Main information
    quote = models.ForeignKey('Quote', verbose_name=_("Facture"), on_delete=models.CASCADE)
    ...
    class Meta:
        verbose_name = _("Devis (Produit)")
        verbose_name_plural = _("Devis (Produits)")

    def __str__(self):
        return "%s - %s" % (self.quote, self.product_description)
...
class Invoice(models.Model):
    ...
    # Quote informations
    quote = models.OneToOneField('Quote', verbose_name=_("Devis"),
        on_delete=models.SET_NULL, blank=True, null=True)
    ...
    discount = models.DecimalField(verbose_name=_("Réduction (en %)"), decimal_places=2, max_digits=5, default=0,
        validators=[MinValueValidator(Decimal('0.00')), MaxValueValidator(Decimal('100'))])
    invoice_pdf = models.FileField(verbose_name=_("Document PDF"), blank=True, null=True)
