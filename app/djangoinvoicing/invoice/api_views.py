...
def customer_infos(request):
    ...
    if request.user.is_authenticated and customer_pk:
        customer_instance = invoice.models.Customer.objects.filter(pk=customer_pk).first()
...
def product_infos(request):
    ...
    if product_pk:
        product_instance = invoice.models.Product.objects.filter(pk=product_pk).first()
        return JsonResponse({
